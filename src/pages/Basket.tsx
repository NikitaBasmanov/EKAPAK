...
const Basket = () => {
    ...
    const handleDelete = (artNumber: number) => {
        const newItems = items.filter(
            (item: Product) => item.artNumber !== artNumber
        );
        setItems(newItems);
    ...
    return (
        ...
        >
            ...
            {items.length > 0 ? (
                <>
                    <div className="w-full sm:w-[50%] text-center py-[20px] sm:py-[30px] lg:py-[40px] flex flex-col sm:flex-row items-center justify-between gap-[20px] sm:gap-[0px]">
                        <div className="flex flex-row items-center justify-start gap-[8px] p-[10px] bg-white rounded-[10px] order-2 sm:order-1">
                            <p>{'<'}</p>
                            <p>Вернуться к покупкам</p>
                        </div>
                        <h1 className="text-[30px] sm:text-[40px] font-bold leading-tight text-nowrap order-1 sm:order-2">
                            Корзина
                        </h1>
                    </div>
                    <div className="w-full flex flex-col lg:flex-row items-start justify-between gap-[20px] lg:gap-[10px]">
                        <div className="w-full lg:w-[70%] bg-white rounded-[20px] p-[15px] sm:p-[20px] lg:p-[30px] flex flex-col items-center justify-between gap-[20px] sm:gap-[30px]">
